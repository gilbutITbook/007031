...
static int most_common_index = 0;
static int most_common_count = 0;
	...
	for (; i < hTable->size; i++){
		if (hTable->flag[hashValue] == EMPTY_NODE || hTable->flag[hashValue] == DELETED_NODE) {
			...
			if (strcmp(buf, value) == 0) {
				...
				if (hTable->count[hashValue] > most_common_count) {
					// 가장 자주 나타나는 단어의 정보를 갱신합니다.
					most_common_index = hashValue;
					most_common_count = hTable->count[hashValue];
				}
